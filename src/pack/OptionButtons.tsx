 ...
 *
 * Each option is its own component so it can drive selection from pointer
 * events (useTap) rather than click — the buttons live inside the quiz's
 * scroll region, where iOS swallows the first tap's click while a
 * momentum scroll is settling.
 */
    ...
    <div className={imageOptions ? 'grid grid-cols-2 gap-2.5' : 'flex flex-col gap-2.5'}>
      {options.map((opt) => (
        <OptionButton
          key={opt.key}
          option={opt}
          isThis={selected === opt.key}
          isAnswer={opt.key === correctKey}
          stage={stage}
          onSelect={onSelect}
        />
      ))}
...
function OptionButton({ option, isThis, isAnswer, stage, onSelect }: OptionButtonProps) {
  const tap = useTap(() => onSelect(option.key));
