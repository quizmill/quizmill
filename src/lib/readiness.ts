 ...
 *
 *  1. FIRST-ATTEMPT ONLY. The mistakes engine re-asks a question until it's
 *     rescued, so lifetime accuracy is inflated toward eventually-correct.
 *     The only fair proxy for cold exam performance is how you did the FIRST
 *     time you ever saw each question.
...
export type ReadinessVerdict = 'no-data' | 'not-ready' | 'borderline' | 'ready';
...
/**
 * First-attempt outcome per question: the earliest attempt (by `answeredAt`)
 * for each questionId, optionally restricted to in-scope questions. This is
 * the cold-performance signal — re-asks after a rescue are ignored.
 */
function firstAttempts(
  attempts: readonly Attempt[],
  inScope: (questionId: string) => boolean,
): Attempt[] {
  const earliest = new Map<string, Attempt>();
  for (const a of attempts) {
    if (!inScope(a.questionId)) continue;
    const cur = earliest.get(a.questionId);
    if (!cur || a.answeredAt < cur.answeredAt) earliest.set(a.questionId, a);
  }
...
export function computeReadiness(
  ...
  const firsts = firstAttempts(attempts, inScope);
  ...
  else if (highPct < goal.passPct) verdict = 'not-ready';
