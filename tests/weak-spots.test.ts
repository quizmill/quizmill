...
describe('buildWeakSpotPool', () => {
  ...
  it('clears a question after a correct streak, despite low all-time accuracy', () => {
    // The reported scenario: wrong, then correct, then correct. The mistake
    // is rescued on the first correct; the second consecutive correct clears
    // the shaky (67% all-time) flag too — so it leaves the pool entirely.
    const b = bank([['q1', 'c']]);
    const attempts = [
      attempt('q1', 'c', false, 3000),
      attempt('q1', 'c', true, 2000),
      attempt('q1', 'c', true, 1000),
    ];
    const out = buildWeakSpotPool(b, attempts, new Set());
    expect(out.orderedIds).toEqual([]);
  });

  it('keeps a question shaky after only one correct (streak of 1)', () => {
    const b = bank([['q1', 'c']]);
    const attempts = [
      attempt('q1', 'c', false, 2000),
      attempt('q1', 'c', true, 1000),
    ];
    const out = buildWeakSpotPool(b, attempts, new Set());
    expect(out.orderedIds).toContain('q1');
  });

  it('re-flags a question if it goes wrong again after being mastered', () => {
    const b = bank([['q1', 'c']]);
    const attempts = [
      attempt('q1', 'c', true, 4000),
      attempt('q1', 'c', true, 3000),
      attempt('q1', 'c', false, 2000), // breaks the streak — back on the list
    ];
    const out = buildWeakSpotPool(b, attempts, new Set());
    expect(out.orderedIds).toContain('q1');
  });
...
describe('masteredQuestionIds', () => {
  it('counts the current trailing run of correct answers', () => {
    const attempts = [
      attempt('q1', 'c', false, 3000),
      attempt('q1', 'c', true, 2000),
      attempt('q1', 'c', true, 1000),
    ];
    expect(masteredQuestionIds(attempts, 2).has('q1')).toBe(true);
    expect(masteredQuestionIds(attempts, 3).has('q1')).toBe(false);
  });

  it('resets the run on a wrong answer', () => {
    const attempts = [
      attempt('q1', 'c', true, 3000),
      attempt('q1', 'c', true, 2000),
      attempt('q1', 'c', false, 1000),
    ];
    expect(masteredQuestionIds(attempts, 2).has('q1')).toBe(false);
  });
});
