...
/**
 * The stress pack is only a local aid, but it still has to be a valid
 * pack — `pack:stress` activates it through `pack:use`, which refuses
 * anything `validatePack` rejects, and a generator that silently drifts
 * out of schema is worse than no generator. Generate a small one and
